...
def add_text_block(doc: Document, text: str):
    """Add text to document, handling markdown formatting"""
    # Clean the text first
    text = clean_markdown_for_docx(text)
    ...
    for chunk in text.split("\n\n"):
        ...
        if '|' in chunk and chunk.count('|') >= 2:
            # Simple table handling - just add as paragraph for now
...
def main():
    ...
    ap = argparse.ArgumentParser(description="OCR PDF → Markdown (text-only, no images).")
    ...
    # Also generate DOCX using python-docx (not Pandoc)
